...
GCar::GCar(const Color& c, const std::string& app, const short& w, const short& h, const short& st, const COORD& p, const RECT& gb)
	: Vehicle()
{
	color		= c;
	appearance	= app;
...
void GCar::Draw(const short& start, const short& end)
{
	...
	for (int cnt = 0, y = 0, i = 0; i < appearance.size(); ++i)
	{
		...
		if (pos.X > GameBoard.left + 2)
		{
			...
			else if (pos.X > GameBoard.left + 2) std::cout << appearance[i];
		}
		else
		{
