...
void Console::OpenMusic(std::wstring filename)
{
	std::wstring cmd = L"open \"" + filename + L"\" type mpegvideo alias mp3";
	mciSendStringW(cmd.c_str(), NULL, 0, NULL);
}
void Console::PlayMusic()
{
...
{
	mciSendStringW((L"setaudio mp3 volume to " + std::to_wstring(volume)).c_str(), NULL, 0, NULL);
}
